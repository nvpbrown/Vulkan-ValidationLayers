...
static void PreCallRecordCreateGraphicsPipelines(const uint32_t count, VkPipeline *pPipelines) {
    for (uint32_t i = 0; i < count; i++) {
...
VKAPI_ATTR VkResult VKAPI_CALL CreateGraphicsPipelines(VkDevice device, VkPipelineCache pipelineCache, uint32_t count,
    ...
    if (skip) {
        PreCallRecordCreateGraphicsPipelines(count, pPipelines);
...
static void PreCallRecordCmdBeginRenderPass(layer_data *dev_data, RENDER_PASS_STATE *render_pass_state, GLOBAL_CB_NODE *cb_state,
                                            FRAMEBUFFER_STATE *framebuffer, const VkRenderPassBeginInfo *pRenderPassBegin,
...
VKAPI_ATTR void VKAPI_CALL CmdExecuteCommands(VkCommandBuffer commandBuffer, uint32_t commandBuffersCount,
    ...
    if (pCB) {
        ...
        skip |= PreCallValidateCmdExecuteCommands(dev_data, pCB, commandBuffer, commandBuffersCount, pCommandBuffers);
        PreCallRecordCmdExecuteCommands(dev_data, pCB, commandBuffersCount, pCommandBuffers);
    }
    lock.unlock();
